    ...
    <div className="admin-page">
      ...
      <div className="admin-grid admin-grid--metrics">
        <div className="admin-card admin-count-card">
          <p className="admin-count-card__label">Monto total vendido</p>
          <p className="admin-count-card__value admin-count-card__value--large">
            {st('montoTotal').cargando ? '...' : fmt(montoTotal)}
          </p>
        </div>
        <div className="admin-card admin-count-card">
          <p className="admin-count-card__label">Restaurante top</p>
          <p className="admin-count-card__value admin-metric__value--small">
            {st('topRest').cargando ? '...' : topRestaurante ? `${topRestaurante.nombre} (${topRestaurante.total_pedidos} pedidos)` : '—'}
          </p>
        </div>
        <div className="admin-card admin-count-card">
          <p className="admin-count-card__label">Hora pico</p>
          <p className="admin-count-card__value admin-count-card__value--large">
            {st('horaPico').cargando ? '...' : horaPico ? `${horaPico.hora}:00` : '—'}
          </p>
        </div>
      </div>
